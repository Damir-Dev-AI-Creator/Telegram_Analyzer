...
async def callback_prompt_help(callback, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"ℹ️ <b>Справка по кастомным промптам</b>\n\n"
        f"<b>Что такое промпт?</b>\n"
        f"Это инструкция для Claude AI, описывающая как анализировать ваши данные.\n\n"
        f"<b>Как использовать:</b>\n"
        f"1. Используйте /setprompt\n"
        f"2. Выберите 'Установить новый'\n"
        f"3. Отправьте текст промпта\n"
        f"4. В промпте используйте <code>{\"csv_content\"}</code> для вставки данных\n\n"
        f"<b>Примеры задач:</b>\n"
        f"• Анализ времени ответа менеджеров\n"
        f"• Выявление частых проблем клиентов\n"
        f"• Статистика по темам обращений\n"
        f"• Оценка качества обслуживания\n\n"
        f"<b>Советы:</b>\n"
        f"✅ Будьте конкретны в инструкциях\n"
        f"✅ Укажите нужный формат вывода\n"
        f"✅ Пишите на русском или английском\n"
        f"❌ Не делайте промпт слишком сложным\n\n"
        f"<b>Команды:</b>\n"
        f"/setprompt - Настроить промпт"
    )
    await callback.answer()
